document.addEventListener("DOMContentLoaded", async () => {
  // document.querySelector("#facts").innerHTML = "<p>Loading...</p>";
  ...
  // const regions = [];
  // facts.forEach((fact) => {
  //   if (!regions.includes(fact.region)) {
  //     regions.push(fact.region);
  //   }
  // });
  // regions.sort();
  ...
  facts.forEach((fact) => {
    region = fact.region;
    // subregion = fact.subregion ? fact.subregion : "—";
    subregion = fact.subregion ?? "—";
    country = fact.name.common;
  ...
  const updateFacts = (region, subregion, country) => {
    ...
    <div id="country-flag">
      <img src="${fact.flags.svg}" alt="Flag of Qatar" />
    </div>
    <table>
      <tbody>
        <tr>
